...
import { createCompany, fetchExchanges } from '../api';
import type { Exchange, CreateCompanyData } from '../api';
...
const CreateCompanyModal = ({ isOpen, onClose, onSuccess }: CreateCompanyModalProps) => {
  ...
  const loadExchanges = async () => {
    ...
    try {
      ...
      setExchanges(data);
      if (data.length > 0) {
        setFormData(prev => ({ ...prev, exchangeId: data[0].id }));
      }
    } catch {
